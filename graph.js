...
'use strict'; (function(){
	...
	for(var k in vObj) {
		v.push({title: k, degree: 0});
		vObj[k] = v.length - 1;
	}
	for(var i=0; i<graph.length; i++) {
		var v0 = vObj[graph[i][0]];
		var v1 = vObj[graph[i][1]];
		v[v0].degree++;
		v[v1].degree++;
		e.push({ source: v0, target: v1 });
	}
	var degreeMax = 1;
	for(var i=0; i<v.length; i++) {
		if(v[i].degree > degreeMax) degreeMax = v[i].degree;
	}
	...
	svgCircles.selectAll('g').data(v).enter().append('g')
		...
		.call(function(g){
			...
			g.append('circle')
				...
				.attr('r', V_CIRCLE_R)
				.each(function(d){
					d3.select(this).style('fill', 'hsl(' + V_CIRCLE_COLOR_H + ',' + V_CIRCLE_COLOR_S + ',' + (75 - d.degree/degreeMax*50) + '%)');
				});
		})
		.call(force.drag);
